...
export class VultrService {
  ...
  encryptFilePath(filePath: string, userId?: number): string {
    const salt = randomBytes(16);
    const iv = randomBytes(16);
    const key = scryptSync(this.secretKeyForS3Image + userId, salt, 32);
  ...
  decryptFilePath(encryptedLink: string, userId?: number): string {
   ...
   */
  generateProxyUrl(filePath: string, userId?: number): string {
    const encryptedLink = this.encryptFilePath(filePath, userId);
